...
def change_work_dir_root(gui, new_root: StrPath = None, logger: Logger = Logger()):
    ...
    if new_root:
        if os.path.isdir(new_root):
            ...
            logger.error(message=f"{new_root} is not a valid directory", error_type=ValueError)
    else:
...
def download_data_node_files(state, data_node_name, files_attribute: str = "scenario.data_nodes", logger: Logger = Logger()):
