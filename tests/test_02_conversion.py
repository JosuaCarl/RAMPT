...
def test_msconv_pipe_run_single():
    ...
    msconvert_runner = MSconvert_Runner( platform=platform, target_format=".mzML",
                                     save_log=False, verbosity=3,
                                     nested=False, workers=1 )
    
    msconvert_runner.run_single( in_path=join(test_path, "minimal_file.mzML"),
                            out_path=out_path )
...
def test_msconv_pipe_run_directory():
    ...
    msconvert_runner.run_directory( in_path=test_path,
                                      out_path=out_path )
...
def test_msconv_pipe_run_cross():
    ...
    msconvert_runner = MSconvert_Runner( platform=platform, target_format=".mzXML",
    ...
    assert msconvert_runner.processed_in == [join(test_path, "minimal_file.mzML"), join(test_path, "nested_test_folder", "minimal_file.mzML")]
    assert msconvert_runner.processed_out == [join(out_path, "minimal_file.mzXML"), join(out_path, "nested_test_folder", "minimal_file.mzXML")]
