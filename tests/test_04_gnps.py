...
def test_gnps_pipe_run_nested():
    ...
    assert os.path.isfile( join( out_path, f"{basename(example_path)}_gnps_all_db_annotations.json") )
    assert os.path.isfile( join( out_path, "example_nested", "example_nested_gnps_all_db_annotations.json") ) 
...
def test_gnps_pipe_run():
    ...
    assert gnps_runner.processed_out == [ join( out_path, f"{basename(example_path)}_gnps_all_db_annotations.json") ]
